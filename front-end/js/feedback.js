...
const TIMESTAMPS_FILE_PATH = "../timestamps.json";
const FEEDBACK_DIR_PATH = "../feedback/";
...
var lastReviewedOcrMsPromise = $.get(TIMESTAMPS_FILE_PATH).then(function(data) {
  return data.ocr_ms;
});
...
export function sendFeedback(photo_id, feedback_type, feedback_obj) {
  ...
  var path = `${FEEDBACK_DIR_PATH}/${photo_id}/${feedback_type}`;
...
export function getFeedbackText(back_id) {
  ...
  lastReviewedOcrMsPromise.then(function(lastReviewedOcrMs) {
    firebaseRef
      .child(FEEDBACK_DIR_PATH + back_id + "/text")
      .orderByKey()
