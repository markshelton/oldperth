...
exports.popular_photos = exports.mapPromise = exports.map = exports.lat_lon_to_marker = undefined;
...
var _mapStyles = __webpack_require__(7);

var _social = __webpack_require__(9);

var _urlState = __webpack_require__(3);
...
var popular_photos = exports.popular_photos = function () {
  var url = "/data.json";
  $.getJSON(url).then(function (response_data) {
    response_data.photos.filter(function (photo) {
...
function initialize_map() {
  var latlng = new google.maps.LatLng(process.env.GOOGLE_MAPS_INITIAL_LATITUDE, process.env.GOOGLE_MAPS_INITIAL_LONGITUDE);
  var opts = {
    zoom: 15,
    maxZoom: 18,
    minZoom: 10,
...
function loadInfoForLatLon(lat_lon) {
  ...
  return $.getJSON(url).then(function (response_data) {
    ...
    if (lat_lon === "pop") {
      filtered_data = response_data.photos.filter(function (photo) {
        return photo.popular;
      });
    } else {
      filtered_data = response_data.photos.filter(function (photo) {
        var photo_lat_lon = photo.latitude + "," + photo.longitude;
        return lat_lon === photo_lat_lon;
      });
    }
    $.extend(photo_id_to_info, filtered_data);
    var photo_ids = filtered_data.map(function (el) {
      return el.photo_id;
    });
    if (lat_lon != "pop") {
...
function findLatLonForPhoto(photo_id, cb) {
  $.ajax({
    ...
    success: function success(data) {
      var entry = data["photos"].find(function (photo) {
        return photo["photo_id"] === photo_id;
      });
      var latlon = entry["latitude"] + "," + entry["longitude"];
      cb(latlon);
    }
  });
}
...
$(function () {
  ...
  var title = function title(state) {
    var main_title = process.env.SITE_TITLE || "Historical Photos";
